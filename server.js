...
// ✅ Multer: recibe la imagen en memoria (sin guardar en disco)
const upload = multer({
...
// ✅ Worker global precargado — se inicializa UNA vez al arrancar el servidor
let worker = null;

async function inicializarWorker() {
  console.log('⏳ Precargando Tesseract...');
  worker = await createWorker('spa+eng'); // Español + inglés para mejor cobertura
  console.log('✅ Tesseract listo');
}

// ── Endpoint: analizar ticket ──────────────────────────────────────────
...
function detectarProductos(lineas) {
  ...
  for (const linea of lineas) {
    ...
    if (matchTotal) {
      total = parsePrecio(matchTotal[1]);
      continue;
    }
    ...
    if (matchPrecio) {
      ...
      if (matchCantidad) {
        productos.push({
          nombre: capitalizarNombre(matchCantidad[2]),
          precio,
...
app.listen(PORT, async () => {
  ...
  await inicializarWorker(); // Precarga Tesseract al arrancar
});
